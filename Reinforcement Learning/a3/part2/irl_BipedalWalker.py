...
class GAIL:
    ...
    def train(self, n_iter):
        ...
        for train_step in range(n_iter+1):
            ...
            #define tensors
            exponTensor = torch.full((self.args.batch_size, 1), 1, device = "cuda:0").float()
            policyTensor = torch.full((self.args.batch_size, 1), 0, device = "cuda:0").float()

            #calculate the loss discriminator
            loss_discriminator = self.loss_fn(self.discriminator(exp_state, exp_action).float(), exponTensor)
            
            #transition policy
            probabilityPolicy = self.discriminator(state, action.detach()).float()

            #add the change after the loss is factored in
            loss_discriminator = self.loss_fn(probabilityPolicy, policyTensor) + loss_discriminator
