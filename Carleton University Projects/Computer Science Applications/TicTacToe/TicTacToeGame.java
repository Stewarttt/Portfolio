...
public class TicTacToeGame{
  ...
  public static int win(int last_row_played, int last_column_played){
    ...
    // check to see if a victory condition is met upon a diagonal plane
    total = 0; 
    ...
    // checks to see how many boxes can be checked in each direction - from bottom to top
    if (rowUp > colRight) {
    ...
    for(int in = 1; in <= possibleQuadOne; ++in) { // checks quadrant 1
      if (board[last_row_played-in][last_column_played+in] == peace) {
        total += 1;
      }
      else { // exits if cross hits an invalid peace
        break; 
      }
    }

    for(int in = 0; in <= possibleQuadThree; ++in) { // quadrant 3
      if (board[last_row_played+in][last_column_played-in] == peace) {
        total += 1;
      }
      else {
    ...
    if (total >= win_condition) { //returns the total win value, if the victory condtions are met
        return total;
    }
    
    // sets total to zero, because were now looking for the other diagonal cross
    total = 0;
    
    // checks to see how many boxes can be checked in each direction - from top to bottom
    if (rowUp > colLeft) { 
    ...
    for(int in = 1; in <= possibleQuadTwo; ++in) { //quadrant 2
      if (board[last_row_played-in][last_column_played-in] == peace) {
        total += 1;
      }
      else {
    ...
    for(int in = 0; in <= possibleQuadFour; ++in) { //quadrant 1
      if (board[last_row_played+in][last_column_played+in] == peace) {
        total += 1;
      }
      else {
    ...
    if (total >= win_condition) { //returns possible win
        return total;
    }
