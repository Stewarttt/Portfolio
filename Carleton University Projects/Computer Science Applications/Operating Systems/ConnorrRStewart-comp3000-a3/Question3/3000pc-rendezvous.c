...
void *thread_start(void *args) {
	shared * s = (shared *) args;
	
	s->con_pid = getpid();

    consumer(s, s->count, s->con_interval);
	return NULL;
}

int main(int argc, char *argv[])
{
        int count, prod_interval, con_interval, error;
		pthread_t tid;
		...
		error = pthread_create(&tid, NULL, thread_start, s);
		if (error != 0) {
			fprintf(stderr, "Could not open thread\n");
		}
